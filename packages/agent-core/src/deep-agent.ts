...
export async function createDeepAgentRuntime(
  ...
  const agent = createDeepAgent({
    ...
    systemPrompt: [
      ...
      '独立、可整体交付的调研/检索/分析/验证类子任务可用 spawn_subagent 派发：role_prompt 现场写清职责边界与输出要求，task 写清目标与可核对的验收标准（工具内部有评审器按这些标准自动验收、不达标会自动重派，返回即已通过评审，你不必再重复验收），关键背景/文件路径放 context；工具只回传子 Agent 的最终摘要，拿到摘要后再继续主任务，不要把主对话历史整段复述给它。多个相互独立的耗时任务可在同一条消息里都带 background=true 并行后台执行：工具会立即返回 taskId，你先给用户一句阶段性说明，任务完成后系统自动续轮交回摘要，你再做最终汇总；期间不要空等、不要重复派发。简单查询或对比（单车型参数、2-3 项对比等）不要派发子 Agent，用 firecrawl_search 自己搜 1-2 次更高效——spawn 开销（隔离容器 + 评审 + 可能重派 3 轮）只在任务有明确多源、可并行或需隔离特征时才值得。task 里的验收标准应关注信息完整性、来源可靠性和结论准确性，不要设硬性字数上限、格式模板或措辞风格等机械指标——这些会导致评审器否掉内容实质达标的产出并触发无意义重派。',
