...
function fakeClient() {
  ...
  return {
    ...
    async upsert(collection: string, body: any) { requests.push({ type: 'upsert', collection, body }); const bucket = points.get(collection) ?? new Map(); points.set(collection, bucket); for (const p of body.points) bucket.set(String(p.id), p); },
...
test('derives distinct collections, merges tenant/kb filters, and upserts idempotently', async () => {
  ...
  assert.deepEqual(client.requests.filter((r: any) => r.type === 'upsert').map((r: any) => r.collection), [c1, c2]);
  assert.deepEqual([...client.points.get(c1)!.keys()], ['x']);
  assert.deepEqual([...client.points.get(c2)!.keys()], ['y']);
  const hits = await a.search([1, 2, 3], 't1', ['k1'], 5);
  assert.deepEqual(hits.map((hit) => hit.chunkId), ['x']);
  assert.deepEqual((await b.search([1, 2, 3], 't1', ['k1'], 5)).map((hit) => hit.chunkId), ['y']);
  assert.deepEqual(client.requests.find((r: any) => r.type === 'search' && r.collection === c1).body.filter.must, [{ key: 'tenant_id', match: { value: 't1' } }, { key: 'kb_id', match: { any: ['k1'] } }]);
});
