...
test('consumer fails claimed job then remains usable for a succeeding job', async () => {
  ...
  class FakeWorker { on() { return this; } constructor(_q: string, fn: any) { handler = fn; } async close() {} }
  const worker: any = startConsumer('knowledge-index', {}, { WorkerClass:FakeWorker, repository:{ claimIndexJob: async () => ({ leaseToken:'lease' }), getDocumentForIndex: async () => ({ object_key:'o', content_hash:'h', size_bytes:1, mime:'text/markdown' }), getKnowledgeBaseForIndex: async () => ({ chunk_size:1000, chunk_overlap:100 }), failIndexJob: async (...args: any[]) => { failed = args; } }, pipeline:{ run: async () => { attempt++; if (attempt === 1) throw new Error('index failed'); } }, metrics:{ indexFailure:() => metrics++ }, logger:{ error:() => logs++ } });
