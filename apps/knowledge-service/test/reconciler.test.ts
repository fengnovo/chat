...
test('reconciler uses database job id and does not duplicate existing queue jobs', async () => {
  ...
  const state: any[] = [{ id:'queued', tenantId:'tenant-a', status:'queued' }, { id:'stale', tenantId:'tenant-b', status:'running', leaseExpiresAt:stale }, { id:'fresh', tenantId:'tenant-c', status:'running', leaseExpiresAt:new Date(now.getTime()+10000) }];
